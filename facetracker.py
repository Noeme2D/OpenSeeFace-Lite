...
# if os.name == 'nt': # nt: new technology, always means windows OS 
...
parser.add_argument("-M", "--mirror-input", action="store_true", help="Process a mirror image of the input video") # Might Need
...
os.environ["OMP_NUM_THREADS"] = '1'; #args.max_threads = 1
...
class OutputLog(object):
    ...
    def flush(self):
        if not self.fh is None:
            self.fh.flush()
        self.output.flush()
# class OutputLog End Here


output_logfile = None
# if args.log_output != "":   #if specified output file
#     output_logfile = open(args.log_output, "w") # open the file for writing
...
import struct
import json
from input_reader import InputReader, VideoReader, DShowCaptureReader, try_int, OpenCVReader
...
target_ip = args.ip #args.ip: IP address for sending tracking data
target_port = args.port #args.port: port for sending tracking data
...
# if os.name == 'nt':
#     dcap = args.dcap #args.dcap: Set which device capability line to use or -1 to use the default camera settings
#     use_dshowcapture_flag = True if args.use_dshowcapture == 1 else False # args.use_dshowcapture: When set to 1, libdshowcapture will be used for video input instead of OpenCV
...
# input_reader = InputReader(0, 0, 640, 360, fps) # args.capture = 0; args.width = 640; args.height = 360; args.raw_rgb = 0
input_reader = OpenCVReader(0, 640, 360, fps) # Only use this specific reader

# if type(input_reader.reader) == VideoReader:
#     fps = 0
if type(input_reader) == VideoReader:
    fps = 0
...
try:
    ...
    while input_reader.is_open():
        ...
        ret, frame = input_reader.read() # width * height * 3
        if ret and args.mirror_input:   # args.mirror_input is used to flip the input video to avoid mirror
        ...
        if first:
            ...
            # args.max_threads = 1; args.threshold = None; args.detection_threshold = 0.6; silent=False if args.silent == 0 else True (args.silent = 0); args.faces = 1; scan_retinaface = 0; args.scan_every = 3; args.discard_after = 10; args.max_feature_updates = 900; args.no_3d_adapt = 1; args.try_hard = 0; args.model = 2
            # no_gaze=False if args.gaze_tracking != 0 else True; args.model_dir = None
            tracker = Tracker(width, height, threshold=None, max_threads=1, max_faces=1, discard_after=10, scan_every=3, silent=False, model_type=2, model_dir=None, no_gaze=True, detection_threshold=0.6, use_retinaface=0, max_feature_updates=900, static_model=True, try_hard= False)
...
if tracking_frames > 0: #args.silent = 0
